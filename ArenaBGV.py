...
def ArenaBGV(puzzleState, puzzleType, puzzleSize):
    ...
    while stateCondition != solvedCondition:
        ...
        if counter == 200:
            counter = -1
            break
        nextState, lastAction = BGVCoupling(nextState, puzzleType, puzzleSize, lastAction, counter)
...
def BGVCoupling(puzzleState, puzzleType, puzzleSize, lastAction, counter):
    ...
    for i in range(len(subsolutions)):
        for j in range(len(subsolutions[i])):
            votes[int(subsolutions[i][j][0])] += len(subsolutions[i][j])
...
# ArenaBGV("WRGWROBBBYBBGGRWWWWGOYOOROOYYGYBGYBROGGWYRRRROOGYWBYWB", "Twisty", 3) # SCRAMBLE ID 1
#ArenaBGV("GOOBROOGRBGOYGBYWBGBBYOORYWRGWWBRORYROYBYWYRWGGWYWRGWB", "Twisty", 3) # SCRAMBLE ID 2

solutionLength = -1
while solutionLength == -1 or solutionLength > 30 or solutionLength == 11:
    solutionLength, solution = ArenaBGV([[3,2,5],[4,8,7],[6,0,1]], "SlidingTile", 8) # Good

    if solutionLength > 11 and solutionLength <= 30:
...
#ArenaBGV([[8,0,7],[4,1,2],[6,3,5]], "SlidingTile", 8)
#BGVCoupling([[0,1,2],[3,4,5],[6,7,8]], "SlidingTile", 8)

# solutionLengths = []
# for i in range(10000):
#     print(i)
#     solutionLengths.append(ArenaBGV([[3,2,5],[4,8,7],[6,0,1]], "SlidingTile", 8)) # Good
# #     #solutionLengths.append(Arena([[8,0,7],[4,1,2],[6,3,5]], "SlidingTile", 8)) # Bad
# #     solutionLengths.append(ArenaPBGV([[3,1,0],[8,2,4],[7,6,5]], "SlidingTile", 8)) # Middle
#     print(solutionLengths)


# Possible midshort: 3,0,6,1,7,2,5,4,8 invert it though
# 8,4,5,2,7,1,6,0,3
# 1, 5, 4, 7, 2, 8, 3, 0, 6
